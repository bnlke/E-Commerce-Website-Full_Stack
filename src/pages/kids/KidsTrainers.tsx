...
export default function KidsTrainers() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Get kids' trainers category ID
        const { data: categories } = await supabase
          .from('product_categories')
          .select('id')
          .ilike('name', '%Kids%Trainers%')
          .limit(1);
        ...
        // Fetch products from kids' trainers category
        const { data, error } = await supabase
          .from('products')
          .select(`
            *,
  ...
  if (loading) {
    return (
      <ShoeLayout
        products={[]}
        title="Kids' Trainers"
        description="Loading our kids' trainers collection..."
        loading={loading}
        error={error}
      />
    );
  ...
  return (
    <ShoeLayout
      products={products}
      title="Kids' Trainers"
      description="Fun and functional trainers designed specifically for growing feet. Our kids' collection features durable materials, easy-on designs, and playful styles that children love."
      loading={loading}
      error={error}
    />
  );
