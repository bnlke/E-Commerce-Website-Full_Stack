...
export default function KidsSchool() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Get kids' school shoes category ID
        ...
        // Fetch products from kids' school shoes category
        const { data, error } = await supabase
          .from('products')
          .select(`
            *,
  ...
  if (loading) {
    return (
      <ShoeLayout
        ...
        description="Loading our kids' school shoes collection..."
        loading={loading}
        error={error}
      />
    );
  ...
  return (
    <ShoeLayout
      products={products}
      title="Kids' School Shoes"
      description="School-approved footwear that combines style with practicality. Our school collection features durable materials, comfortable fit, and designs that meet uniform requirements."
      loading={loading}
      error={error}
    />
  );
