...
export default function KidsSummer() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Get kids' summer category ID
        const { data: categories } = await supabase
          .from('product_categories')
          .select('id')
          .ilike('name', '%Kids%Summer%')
          .limit(1);
        ...
        // Fetch products from kids' summer category
        const { data, error } = await supabase
          .from('products')
          .select(`
            *,
  ...
  if (loading) {
    return (
      <ShoeLayout
        ...
        description="Loading our summer collection..."
        loading={loading}
        error={error}
      />
    );
  ...
  return (
    <ShoeLayout
      products={products}
      title="Summer Fun Collection"
      description="Light and breathable shoes perfect for summer adventures. From playgrounds to beach days, keep their feet comfortable all summer long."
      loading={loading}
      error={error}
    />
  );
