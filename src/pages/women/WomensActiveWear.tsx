...
export default function WomensActiveWear() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Get women's activewear category ID
        const { data: categories } = await supabase
          .from('product_categories')
          .select('id')
          .ilike('name', '%Women%Active%')
          .limit(1);
        ...
        // Fetch products from women's activewear category
        const { data, error } = await supabase
          .from('products')
          .select(`
            *,
  ...
  if (loading) {
    return (
      <ShoeLayout
        ...
        description="Loading our women's activewear collection..."
        loading={loading}
        error={error}
      />
    );
  ...
  return (
    <ShoeLayout
      products={products}
      title="Women's Activewear"
      description="High-performance activewear designed to move with you. Our collection features moisture-wicking fabrics and comfortable fits for every workout."
      loading={loading}
      error={error}
    />
  );
