...
import { Heart, ShoppingBag, Truck, RotateCcw, HeartOff, ArrowRight } from 'lucide-react';
...
export default function ProductDetails() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        // Fetch product details with error handling for no results
  ...
  const addToCart = async () => {
    ...
    try {
      // Direct update to product_sizes table
      const { error: updateError } = await supabase
        .from('product_sizes')
        .update({ stock_quantity: sizeStock.stock_quantity - 1 })
          ...
          <div>
            ...
            <div className="flex space-x-4 mb-4">
              <button
                ...
                className="flex-1 bg-black text-white py-4 rounded-md font-medium flex items-center justify-center space-x-2 hover:bg-gray-900 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
