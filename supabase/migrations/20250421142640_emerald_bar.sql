/*
  ...
  1. Changes
    ...
    - Add function to get primary product ID
    - Update stock handling to prevent doubling
    - Fix synchronization across views
*/
...
BEGIN
  ...
  -- Get current quantity from primary product
  ...
  SET stock_status = (
    SELECT 
      CASE
        ...
        WHEN v_new_quantity <= 10 THEN 'low_stock'
        ELSE 'in_stock'
      END
  )
  ...
  RAISE NOTICE 'Stock restored for product % (primary ID: %), size %: % -> %',
    v_product_name, v_primary_id, OLD.size, v_current_quantity, v_new_quantity;
...
BEGIN
  ...
  IF NEW.product_id = v_primary_id THEN
    ...
    -- Update stock status for all instances
    UPDATE products p
    SET stock_status = (
      SELECT 
        CASE
